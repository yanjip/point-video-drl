...
# 7.11日 
- 又写了个粗粒度分配的baseline,并可视化展示，结果显示效果还行
- Tslot只能调0.1-0.2 大于0.2后，所提算法的性能低于baseline
- 修改终端解码能力的大小时，所提算法的耗时太大了，不适用，因此计算得到的QoE没有参考价值。

# 7.12日
- 修改计算能力的值，重新训练一下可以得到正确的结果；
同时结果显示，计算能力值调小后，uncompress算法结果接近所提算法，符合常理。

# 7.13日
- 完成了quality VS Tslot和 VS computation capacity的结果可视化。
- 还存在的问题：①coarse算法居然是第二高的；②如何解决proposed算法的time大于1；
