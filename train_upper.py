'''
@Author  ：Yan JP
@Created on Date：2023/5/22 15:40 
'''
import torch
import numpy as np
import os
...
class Runner():
    def __init__(self, args, ttile: tile, fov_id):
        ...
        self.env = env.upperEnvironment(ttile, fov_id)
        ...
        self.train_log_dir = 'runs/DQN_upper/' + current_time
    ...
    def run(self, ):
        ...
        while self.total_steps < self.args.max_train_steps:
            ...
            episode_reward = 0
            while not done:
                action = self.agent_upper.choose_action(state, epsilon=self.epsilon)
                episode_steps += 1
                ...
                if self.replay_buffer.current_size >= self.args.batch_size:
                    self.agent_upper.learn(self.replay_buffer, self.total_steps)
    ...
    def evaluate_policy(self, ):
        evaluate_reward = 0
        self.agent_upper.net.eval()  # 模型不会更新参数，也不会使用一些只在训练时有效的层，例如dropout或batch normalization。这样可以提高模型的预测性能和稳定性。
        res = []
        for i in range(self.args.evaluate_times):
            ...
            episode_steps = 0
            while not done:
                action = self.agent_upper.choose_action(state, epsilon=0)
                res.append(action)
                episode_steps += 1
                next_state, reward, done, _ = self.env.step(action, episode_steps)
                episode_reward += reward
        ...
        print("-----------------total_steps:{} \t evaluate_reward:{} \t epsilon：{}".format(self.total_steps,
                                                                                           evaluate_reward,
                                                                                           self.epsilon))
        infoBt = self.env.get_info()
        print("\t\t----用户缓冲区状态：", infoBt)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_train_steps", type=int, default=int(1.0e4), help=" Maximum number of training steps")
